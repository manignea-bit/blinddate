...
import { getFirestore, doc, getDoc, setDoc, updateDoc, addDoc, deleteDoc, collection, query, where, orderBy, onSnapshot, serverTimestamp, increment } from "firebase/firestore";
...
const BONUS_TYPES=[{id:"time",icon:"⏱️",name:"+30s"},{id:"city",icon:"📍",name:"Ville"},{id:"ice",icon:"🎲",name:"Brise-glace"},{id:"peek",icon:"👀",name:"Intérêt"},{id:"double",icon:"💫",name:"2x match"}];
const DEF_BONUS={time:3,city:3,ice:3,peek:3,double:3};
...
useEffect(()=>{if(!sel)return;const q=query(collection(db,"matches",sel.matchId,"messages"),orderBy("createdAt","asc"));const un=onSnapshot(q,s=>sM(s.docs.map(d=>({id:d.id,...d.data()}))));return()=>un()},[sel]);
...
<button onClick={()=>{sS(null);sM([])}} style={{background:"transparent",border:"none",color:T.textS,fontSize:20,cursor:"pointer"}}>←</button>
...
{matches.map((m,i)=><Card key={m.matchId} hover style={{padding:"16px 20px",marginBottom:12,cursor:"pointer"}}><div onClick={()=>sS(m)} style={{display:"flex",alignItems:"center",gap:14}}>
...
// ═══ BLIND CHAT — BONUS SYNCED ═══
function BlindChat({chatId,myUid,partner,bonuses,onUseBonus,onTimeUp}){const T=useT();const[msgs,sM]=useState([]);const[inp,sI]=useState("");const[tl,sTl]=useState(CHAT_DUR);const[reveals,sR]=useState([]);const cr=useRef(null);const ir=useRef(null);const processedBonuses=useRef(new Set());

// Listen messages + bonus time sync
useEffect(()=>{const q=query(collection(db,"blindChats",chatId,"messages"),orderBy("createdAt","asc"));
return onSnapshot(q,s=>{const all=s.docs.map(d=>({id:d.id,...d.data()}));sM(all);
// Check for bonus time messages from the OTHER player
s.docChanges().forEach(c=>{if(c.type==="added"){const d=c.doc.data();const id=c.doc.id;
if(d.bonusTime&&d.senderId!==myUid&&!processedBonuses.current.has(id)){processedBonuses.current.add(id);sTl(t=>t+d.bonusTime)}}})
})},[chatId,myUid]);
...
const count=bonuses[id]||0;if(count<=0)return;
// Consume bonus
onUseBonus(id);

if(id==="time"){sTl(s=>s+30);
// Send system message with bonusTime so the other player gets +30s too
await addDoc(collection(db,"blindChats",chatId,"messages"),{senderId:"system",text:"⏱️ +30 secondes ajoutées !",createdAt:serverTimestamp(),bonusTime:30})}
...
if(id==="double"){sR(r=>[...r,"💫 Double match activé !"])}}
...
const[scr,sScr]=useState("loading");const[tab,sTab]=useState("home");const[user,sUser]=useState(null);const[prof,sProf]=useState(null);const[cid,sCid]=useState(null);const[ouid,sOuid]=useState(null);const[oprof,sOprof]=useState(null);const[opartner,sOp]=useState(null);const[matches,sMatches]=useState([]);const[bonuses,sBonuses]=useState(DEF_BONUS);const[hasDouble,sHD]=useState(false);
...
function lm(uid){um.current?.();const q=query(collection(db,"matches"),where("users","array-contains",uid));um.current=onSnapshot(q,async s=>{const seen=new Set();const r=[];for(const d of s.docs){const dt=d.data();const oid=dt.users.find(id=>id!==uid);
// FIX: prevent duplicate matches
const key=[uid,oid].sort().join("-");if(seen.has(key))continue;seen.add(key);
const us=await getDoc(doc(db,"users",oid));r.push({matchId:d.id,...dt,...(us.exists()?us.data():{}),otherId:oid})}sMatches(r)})}

async function startChat(){sScr("waiting");sHD(false);const myUid=user.uid;
...
// Only user with smaller UID creates the chat (prevents duplicates)
if(myUid<p.uid){const ref=await addDoc(collection(db,"blindChats"),{users:[myUid,p.uid],status:"active",user1Decision:null,user2Decision:null,createdAt:serverTimestamp()});
...
// Other user waits for the chat to be created
const checkChat=()=>{const cq=query(collection(db,"blindChats"),where("users","array-contains",myUid),where("status","==","active"));
const cu=onSnapshot(cq,cs=>{const f=cs.docs.find(d=>d.data().users.includes(p.uid));if(f){cu();sCid(f.id);sOuid(p.uid);sScr("chat")}});
setTimeout(()=>cu(),15000)};checkChat()}})}

function cancelW(){uw.current?.();deleteDoc(doc(db,"waitingRoom",user.uid)).catch(()=>{});sScr("main")}
const onTU=useCallback(()=>sScr("decision"),[]);

// Consume bonus and save to Firestore
async function consumeBonus(id){const nb={...bonuses,[id]:Math.max(0,(bonuses[id]||0)-1)};sBonuses(nb);
if(id==="double")sHD(true);
...
const isMatch=mine==="match"&&(theirs==="match"||hasDouble);
if(isMatch){
// Only one player creates the match (smaller UID)
...
{scr==="decision"&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",padding:40,textAlign:"center",animation:"fadeIn .5s"}}><div style={{fontSize:56,marginBottom:24,animation:"float 2s ease-in-out infinite"}}>⏰</div><h2 style={{fontFamily:"'Playfair Display',serif",fontSize:28,color:T2.text,margin:"0 0 10px"}}>Temps écoulé !</h2>{hasDouble&&<div style={{padding:"8px 16px",borderRadius:12,background:T2.goldSoft,border:`1px solid ${T2.gold}33`,fontFamily:"'Nunito'",fontSize:13,fontWeight:700,color:T2.gold,marginBottom:16}}>💫 Double match actif !</div>}<p style={{fontFamily:"'Nunito'",fontSize:15,color:T2.textS,marginBottom:40}}>Tu veux découvrir qui se cache derrière ?</p><div style={{display:"flex",gap:16}}><Btn onClick={()=>decide("match")} style={{padding:"18px 40px",fontSize:17}}>💕 Matcher</Btn><Btn variant="ghost" onClick={()=>decide("pass")} style={{padding:"18px 40px",fontSize:17}}>Passer →</Btn></div></div>}
