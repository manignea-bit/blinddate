...
// XP System
const XP_PER_CHAT=5;const XP_PER_MATCH=20;const XP_PER_MSG=1;
function getLevel(xp){if(xp<50)return{level:1,name:"Débutant",next:50};if(xp<150)return{level:2,name:"Curieux",next:150};if(xp<300)return{level:3,name:"Sociable",next:300};if(xp<500)return{level:4,name:"Charmeur",next:500};if(xp<800)return{level:5,name:"Séducteur",next:800};if(xp<1200)return{level:6,name:"Expert",next:1200};return{level:7,name:"Légende",next:99999}}
...
// ═══ UI ═══
function Btn({children,variant="primary",disabled,onClick,style:sx,full}){const[h,sH]=useState(false);const T=useT();const v={primary:{bg:T.accentGrad,c:"#fff",b:"none",s:`0 4px 28px ${T.accentGlow}`},ghost:{bg:"transparent",c:T.textS,b:`1px solid ${T.border}`,s:"none"},google:{bg:T.gBg,c:T.gC,b:T.gB,s:"0 2px 8px rgba(0,0,0,0.06)"},danger:{bg:`${T.danger}15`,c:T.danger,b:`1px solid ${T.danger}33`,s:"none"},gold:{bg:`linear-gradient(135deg,${T.gold},#f59e0b)`,c:"#fff",b:"none",s:"0 4px 20px rgba(251,191,36,0.3)"}}[variant]||{bg:T.accentGrad,c:"#fff",b:"none",s:`0 4px 28px ${T.accentGlow}`};return<button disabled={disabled} onClick={onClick} onMouseEnter={()=>sH(true)} onMouseLeave={()=>sH(false)} style={{padding:"14px 28px",fontSize:15,fontWeight:700,fontFamily:"'Nunito',sans-serif",borderRadius:16,cursor:disabled?"not-allowed":"pointer",opacity:disabled?.5:1,background:v.bg,color:v.c,border:v.b,boxShadow:v.s,width:full?"100%":"auto",transition:"all .35s cubic-bezier(.34,1.56,.64,1)",transform:h&&!disabled?"translateY(-3px) scale(1.04)":"none",filter:h&&!disabled?"brightness(1.1)":"none",...sx}}>{children}</button>}
...
function Timer({sec,tot}){const T=useT();const p=Math.min((sec/Math.max(tot,1))*100,100),lo=sec<=10;return<div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:120,height:6,borderRadius:3,background:T.border,overflow:"hidden"}}><div style={{width:`${p}%`,height:"100%",borderRadius:3,background:lo?T.danger:T.accentGrad,transition:"width 0.5s"}}/></div><span style={{fontFamily:"'Nunito'",fontWeight:800,fontSize:14,color:lo?T.danger:T.accent,fontVariantNumeric:"tabular-nums",animation:lo?"pulse 1s infinite":"none"}}>{sec}s</span></div>}

function XPBar({xp}){const T=useT();const lv=getLevel(xp||0);const pct=((xp||0)/lv.next)*100;
return<div style={{display:"flex",alignItems:"center",gap:10}}>
<div style={{width:28,height:28,borderRadius:"50%",background:T.accentGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:"#fff",fontFamily:"'Nunito'"}}>{lv.level}</div>
<div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontFamily:"'Nunito'",fontSize:11,fontWeight:700,color:T.text}}>{lv.name}</span><span style={{fontFamily:"'Nunito'",fontSize:10,color:T.textD}}>{xp||0}/{lv.next} XP</span></div>
<div style={{height:4,borderRadius:2,background:T.border,overflow:"hidden"}}><div style={{width:`${Math.min(pct,100)}%`,height:"100%",background:T.accentGrad,transition:"width .5s"}}/></div></div></div>}
...
// Report modal
function ReportModal({onClose,onReport}){const T=useT();const[reason,sR]=useState("");
const reasons=["Comportement inapproprié","Spam","Faux profil","Harcèlement","Contenu offensant","Autre"];
return<div style={{position:"fixed",inset:0,zIndex:200,background:T.overlay,backdropFilter:"blur(16px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"fadeIn .3s"}}>
<Card style={{padding:28,maxWidth:380,width:"100%"}}>
<h3 style={{fontFamily:"'Nunito'",fontSize:18,fontWeight:800,color:T.text,marginBottom:16}}>🚩 Signaler</h3>
<div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
{reasons.map(r=><button key={r} onClick={()=>sR(r)} style={{padding:"12px 16px",borderRadius:14,border:`1.5px solid ${reason===r?T.danger:T.border}`,background:reason===r?`${T.danger}10`:"transparent",color:reason===r?T.danger:T.textS,fontFamily:"'Nunito'",fontSize:14,fontWeight:600,cursor:"pointer",textAlign:"left",transition:"all .2s"}}>{r}</button>)}
</div>
<div style={{display:"flex",gap:10}}>
<Btn variant="danger" full onClick={()=>{if(reason)onReport(reason)}} disabled={!reason}>Signaler</Btn>
...
async function sub(){sErr("");sLd(true);try{if(m==="signup"){if(!n.trim()){sErr("Prénom requis");sLd(false);return}if(p.length<6){sErr("6 car. min");sLd(false);return}const c=await createUserWithEmailAndPassword(auth,e,p);await updateProfile(c.user,{displayName:n.trim()});await setDoc(doc(db,"users",c.user.uid),{name:n.trim(),email:e,age:null,city:"",bio:"",photos:[],interests:[],profileComplete:false,bonuses:DEF_BONUS,xp:0,blocked:[],location:null,createdAt:serverTimestamp()});onAuth(c.user)}else{await signInWithEmailAndPassword(auth,e,p)}}catch(er){const ms={"auth/email-already-in-use":"Email déjà utilisé","auth/user-not-found":"Aucun compte","auth/wrong-password":"Mauvais mdp","auth/invalid-credential":"Email ou mdp incorrect"};sErr(ms[er.code]||er.message)}sLd(false)}
async function ggl(){sErr("");sLd(true);try{const r=await signInWithPopup(auth,gProv);const u=r.user;const s=await getDoc(doc(db,"users",u.uid));if(!s.exists())await setDoc(doc(db,"users",u.uid),{name:u.displayName||"",email:u.email,age:null,city:"",bio:"",photos:u.photoURL?[u.photoURL]:[],interests:[],profileComplete:false,bonuses:DEF_BONUS,xp:0,blocked:[],location:null,createdAt:serverTimestamp()})}catch(er){sErr(er.message)}sLd(false)}
...
// ═══ SETUP ═══
function Setup({user,onDone}){const T=useT();const[st,sSt]=useState(0);const[age,sA]=useState("");const[city,sC]=useState("");const[bio,sB]=useState("");const[ints,sI]=useState([]);const[pf,sPf]=useState(null);const[pp,sPp]=useState(null);const[up,sUp]=useState(false);const[err,sErr]=useState("");const[locating,sLoc]=useState(false);const fr=useRef(null);

// Auto-detect city from geolocation
async function detectLocation(){sLoc(true);try{const pos=await new Promise((res,rej)=>navigator.geolocation.getCurrentPosition(res,rej,{timeout:10000}));const{latitude:lat,longitude:lng}=pos.coords;
const r=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`);const d=await r.json();
const cityName=d.address?.city||d.address?.town||d.address?.village||d.address?.municipality||"";
if(cityName)sC(cityName);
await updateDoc(doc(db,"users",user.uid),{location:{lat,lng}});
}catch(e){console.log("Geo error:",e)}sLoc(false)}
...
{st===0&&<><label style={{fontFamily:"'Nunito'",fontSize:13,fontWeight:600,color:T.textS,display:"block",marginBottom:8}}>Âge</label><input type="number" min={18} value={age} onChange={e=>sA(e.target.value)} placeholder="18" style={{...inp,width:120,marginBottom:18}}/><label style={{fontFamily:"'Nunito'",fontSize:13,fontWeight:600,color:T.textS,display:"block",marginBottom:8}}>Ville</label><div style={{display:"flex",gap:8}}><input value={city} onChange={e=>sC(e.target.value)} placeholder="Paris, Lyon..." style={{...inp,flex:1,marginBottom:0}}/><button onClick={detectLocation} disabled={locating} style={{padding:"12px 16px",borderRadius:14,background:T.accentSoft,border:`1px solid ${T.accent}33`,color:T.accent,fontSize:13,fontFamily:"'Nunito'",fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>{locating?"...":"📍 Auto"}</button></div></>}
...
// ═══ HOME ═══
function HomeTab({profile,onStart,bonuses}){const T=useT();const lv=getLevel(profile.xp||0);
...
{[{i:"🎭",t:"Chat anonyme",d:"Pas de photo, pas de nom"},{i:"⏱️",t:"60 secondes",d:"Utilise tes bonus !"},{i:"💕",t:"Matcher",d:"+20 XP par match réussi"}].map((s,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:14,padding:"12px 0",borderTop:i?`1px solid ${T.border}`:"none"}}><span style={{fontSize:24}}>{s.i}</span><div><div style={{fontFamily:"'Nunito'",fontSize:14,fontWeight:700,color:T.text}}>{s.t}</div><div style={{fontFamily:"'Nunito'",fontSize:12,color:T.textD}}>{s.d}</div></div></div>)}</Card></div>}

// ═══ MATCHES ═══
function MatchesTab({myUid,matches,onBlock}){const T=useT();const[sel,sS]=useState(null);const[msgs,sM]=useState([]);const[inp,sI]=useState("");const[report,sReport]=useState(false);const cr=useRef(null);const ir=useRef(null);
...
async function handleReport(reason){await addDoc(collection(db,"reports"),{reporter:myUid,reported:sel.otherId,reason,matchId:sel.matchId,createdAt:serverTimestamp()});sReport(false);alert("Signalement envoyé. Merci !")}
async function handleBlock(){if(!confirm(`Bloquer ${sel.name} ? Vous ne pourrez plus être mis en contact.`))return;await onBlock(sel.otherId);sS(null)}
...
<div style={{flex:1}}><div style={{fontFamily:"'Nunito'",fontSize:15,fontWeight:700,color:T.text}}>{sel.name}</div></div>
<button onClick={()=>sReport(true)} style={{background:"transparent",border:"none",fontSize:16,cursor:"pointer",padding:4}} title="Signaler">🚩</button>
<button onClick={handleBlock} style={{background:"transparent",border:"none",fontSize:16,cursor:"pointer",padding:4}} title="Bloquer">🚫</button>
</div>
...
async function updateLocation(){try{const pos=await new Promise((res,rej)=>navigator.geolocation.getCurrentPosition(res,rej,{timeout:10000}));const{latitude:lat,longitude:lng}=pos.coords;
const r=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`);const d=await r.json();const cityName=d.address?.city||d.address?.town||d.address?.village||"";
if(cityName){sCt(cityName);await updateDoc(doc(db,"users",user.uid),{city:cityName,location:{lat,lng}});setProfile(p=>({...p,city:cityName,location:{lat,lng}}))}}catch{alert("Impossible de détecter ta position")}}
...
if(id==="time"){const sn=await getDoc(doc(db,"blindChats",chatId));const cur=sn.data()?.endTime;if(cur){const ne=new Date(new Date(cur).getTime()+30000).toISOString();await updateDoc(doc(db,"blindChats",chatId),{endTime:ne})}await addDoc(collection(db,"blindChats",chatId,"messages"),{senderId:"system",text:"⏱️ +30 secondes ajoutées !",createdAt:serverTimestamp()})}
...
function lm(uid){um.current?.();const q=query(collection(db,"matches"),where("users","array-contains",uid));um.current=onSnapshot(q,async s=>{const seen=new Set();const r=[];for(const d of s.docs){const dt=d.data();const oid=dt.users.find(id=>id!==uid);const key=[uid,oid].sort().join("-");if(seen.has(key))continue;seen.add(key);const us=await getDoc(doc(db,"users",oid));const udata=us.exists()?us.data():{};
// Filter out blocked users
if(udata.blocked?.includes(uid))continue;
const myData=await getDoc(doc(db,"users",uid));if(myData.exists()&&myData.data().blocked?.includes(oid))continue;
r.push({matchId:d.id,...dt,...udata,otherId:oid})}sMatches(r)})}

async function blockUser(otherId){await updateDoc(doc(db,"users",user.uid),{blocked:arrayUnion(otherId)});sProf(p=>({...p,blocked:[...(p.blocked||[]),otherId]}));lm(user.uid)}

async function addXP(amount){const newXP=(prof?.xp||0)+amount;sProf(p=>({...p,xp:newXP}));await updateDoc(doc(db,"users",user.uid),{xp:newXP})}
...
await addXP(XP_PER_CHAT);sCid(ref.id);sOuid(p.uid);sScr("chat")}else{
const cq=query(collection(db,"blindChats"),where("users","array-contains",myUid),where("status","==","active"));
const cu=onSnapshot(cq,cs=>{const f=cs.docs.find(d=>d.data().users.includes(p.uid));if(f){cu();addXP(XP_PER_CHAT);sCid(f.id);sOuid(p.uid);sScr("chat")}});setTimeout(()=>cu(),20000)}})}
...
await addXP(XP_PER_MATCH);const s=await getDoc(doc(db,"users",ouid));sOprof(s.exists()?s.data():{name:"?"});sScr("matchReveal")}else sScr("noMatch")}
