...
const CHAT_DUR=60;const SPEED_DUR=30;const SPEED_ROUNDS=3;const DECISION_TIMEOUT=30;
const INTS=["Cinéma","Musique","Sport","Voyages","Art","Tech","Cuisine","Lecture","Gaming","Photo","Nature","Mode","Humour","Science"];
const ICEBREAKERS=["Tu préfères voyager dans le passé ou le futur ?","C'est quoi ton guilty pleasure ?","Si t'avais un superpouvoir ?","Ton emoji préféré ?","Le dernier truc qui t'a fait rire ?","Un lieu de rêve pour un premier date ?","Chien ou chat ?","Ta chanson du moment ?"];
...
function getLv(xp){if(xp<50)return{lv:1,n:"Débutant",nx:50};if(xp<150)return{lv:2,n:"Curieux",nx:150};if(xp<300)return{lv:3,n:"Sociable",nx:300};if(xp<500)return{lv:4,n:"Charmeur",nx:500};if(xp<800)return{lv:5,n:"Séducteur",nx:800};return{lv:6,n:"Légende",nx:9999}}
...
function Btn({children,variant="primary",disabled,onClick,style:sx,full}){const[h,sH]=useState(false);const T=useT();const v={primary:{bg:T.accentGrad,c:"#fff",b:"none",s:`0 4px 28px ${T.accentGlow}`},ghost:{bg:"transparent",c:T.textS,b:`1px solid ${T.border}`,s:"none"},google:{bg:T.gBg,c:T.gC,b:T.gB,s:"0 2px 8px rgba(0,0,0,0.06)"},danger:{bg:`${T.danger}15`,c:T.danger,b:`1px solid ${T.danger}33`,s:"none"},sec:{bg:`linear-gradient(135deg,${T.sec},#a66bff)`,c:"#fff",b:"none",s:`0 4px 20px ${T.secGlow}`}}[variant]||{bg:T.accentGrad,c:"#fff",b:"none",s:`0 4px 28px ${T.accentGlow}`};return<button disabled={disabled} onClick={onClick} onMouseEnter={()=>sH(true)} onMouseLeave={()=>sH(false)} style={{padding:"14px 28px",fontSize:15,fontWeight:700,fontFamily:"'Nunito',sans-serif",borderRadius:16,cursor:disabled?"not-allowed":"pointer",opacity:disabled?.5:1,background:v.bg,color:v.c,border:v.b,boxShadow:v.s,width:full?"100%":"auto",transition:"all .35s cubic-bezier(.34,1.56,.64,1)",transform:h&&!disabled?"translateY(-3px) scale(1.04)":"none",...sx}}>{children}</button>}
function Card({children,style:sx,hover,glow}){const[h,sH]=useState(false);const T=useT();return<div onMouseEnter={()=>hover&&sH(true)} onMouseLeave={()=>hover&&sH(false)} style={{background:T.card,borderRadius:24,border:`1px solid ${T.border}`,boxShadow:h?`0 12px 40px rgba(0,0,0,.3),0 0 20px ${glow||T.accentGlow}`:"0 4px 20px rgba(0,0,0,.15)",transition:"all .35s cubic-bezier(.34,1.56,.64,1)",transform:h?"translateY(-4px)":"none",...sx}}>{children}</div>}
function Timer({sec,tot}){const T=useT();const p=Math.min((sec/Math.max(tot,1))*100,100),lo=sec<=10;return<div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:100,height:6,borderRadius:3,background:T.border,overflow:"hidden"}}><div style={{width:`${p}%`,height:"100%",borderRadius:3,background:lo?T.danger:T.accentGrad,transition:"width .5s"}}/></div><span style={{fontFamily:"'Nunito'",fontWeight:800,fontSize:14,color:lo?T.danger:T.accent,fontVariantNumeric:"tabular-nums",animation:lo?"pulse 1s infinite":"none"}}>{sec}s</span></div>}
function XPBar({xp}){const T=useT();const l=getLv(xp||0);const p=((xp||0)/l.nx)*100;return<div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:28,height:28,borderRadius:"50%",background:T.accentGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:"#fff",fontFamily:"'Nunito'"}}>{l.lv}</div><div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontFamily:"'Nunito'",fontSize:11,fontWeight:700,color:T.text}}>{l.n}</span><span style={{fontFamily:"'Nunito'",fontSize:10,color:T.textD}}>{xp||0}/{l.nx}</span></div><div style={{height:4,borderRadius:2,background:T.border,overflow:"hidden"}}><div style={{width:`${Math.min(p,100)}%`,height:"100%",background:T.accentGrad}}/></div></div></div>}
...
function PhotoSlot({url,onUp,onRm,idx}){const T=useT();const ref=useRef(null);const[h,sH]=useState(false);return<div onMouseEnter={()=>sH(true)} onMouseLeave={()=>sH(false)} onClick={()=>!url&&ref.current?.click()} style={{aspectRatio:"3/4",borderRadius:20,overflow:"hidden",position:"relative",cursor:"pointer",border:url?`2px solid ${T.border}`:`2px dashed ${T.borderL}`,background:url?"transparent":T.surfAlt,transition:"all .3s",transform:h?"scale(1.03)":"none"}}>{url?<><img src={url} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/>{h&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><button onClick={e=>{e.stopPropagation();ref.current?.click()}} style={{padding:"8px 14px",borderRadius:12,background:T.accentGrad,border:"none",color:"#fff",fontSize:12,fontFamily:"'Nunito'",fontWeight:700,cursor:"pointer"}}>📷</button><button onClick={e=>{e.stopPropagation();onRm(idx)}} style={{padding:"8px 14px",borderRadius:12,background:`${T.danger}33`,border:"none",color:T.danger,fontSize:12,fontFamily:"'Nunito'",fontWeight:700,cursor:"pointer"}}>✕</button></div>}</>:<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%"}}><span style={{fontSize:28,color:T.textD}}>＋</span><span style={{fontFamily:"'Nunito'",fontSize:11,color:T.textD}}>Photo {idx+1}</span></div>}<input ref={ref} type="file" accept="image/*" onChange={e=>{const f=e.target.files?.[0];if(f){if(f.size>5e6){alert("Max 5Mo");return}onUp(idx,f)}}} style={{display:"none"}}/></div>}

// ═══ REPORT ═══
function ReportModal({T,onClose,onReport}){const[r,sR]=useState("");const reasons=["Comportement inapproprié","Spam","Faux profil","Harcèlement","Contenu offensant","Autre"];return<div style={{position:"fixed",inset:0,zIndex:200,background:T.overlay,backdropFilter:"blur(16px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"fadeIn .3s"}}><div style={{background:T.card,borderRadius:24,border:`1px solid ${T.border}`,padding:28,maxWidth:380,width:"100%"}}><h3 style={{fontFamily:"'Nunito'",fontSize:18,fontWeight:800,color:T.text,marginBottom:16}}>🚩 Signaler</h3><div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>{reasons.map(x=><button key={x} onClick={()=>sR(x)} style={{padding:"12px 16px",borderRadius:14,border:`1.5px solid ${r===x?T.danger:T.border}`,background:r===x?`${T.danger}10`:"transparent",color:r===x?T.danger:T.textS,fontFamily:"'Nunito'",fontSize:14,fontWeight:600,cursor:"pointer",textAlign:"left"}}>{x}</button>)}</div><div style={{display:"flex",gap:10}}><Btn variant="danger" full onClick={()=>{if(r)onReport(r)}} disabled={!r}>Signaler</Btn><Btn variant="ghost" onClick={onClose}>Annuler</Btn></div></div></div>}

// ═══ MESSAGE BUBBLE ═══
function Msg({m,myUid,T}){const mine=m.senderId===myUid;const sys=m.senderId==="system";
if(sys)return<div style={{display:"flex",justifyContent:"center",marginBottom:8}}><div style={{padding:"10px 18px",borderRadius:16,background:`${T.sec}15`,border:`1px solid ${T.sec}33`,fontFamily:"'Nunito'",fontSize:13,color:T.sec,fontWeight:600,maxWidth:"85%"}}>{m.text}</div></div>;
return<div style={{display:"flex",justifyContent:mine?"flex-end":"flex-start",marginBottom:8,animation:"slideUp .3s"}}><div style={{maxWidth:"75%",padding:"10px 16px",borderRadius:22,borderBottomRightRadius:mine?6:22,borderBottomLeftRadius:mine?22:6,background:mine?T.accentGrad:T.surfAlt,color:mine?"#fff":T.text,fontSize:14,lineHeight:1.5,fontFamily:"'Nunito'",boxShadow:mine?`0 4px 16px ${T.accentGlow}`:"none"}}>{m.text}</div></div>}
...
async function detectCity(){try{const pos=await new Promise((r,j)=>navigator.geolocation.getCurrentPosition(r,j,{timeout:10000}));const resp=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&format=json`);const d=await resp.json();const c=d.address?.city||d.address?.town||d.address?.village||"";if(c)sC(c)}catch{}}
...
{st===0&&<><label style={{fontFamily:"'Nunito'",fontSize:13,fontWeight:600,color:T.textS,display:"block",marginBottom:8}}>Âge</label><input type="number" min={18} value={age} onChange={e=>sA(e.target.value)} placeholder="18" style={{...inp,width:120,marginBottom:18}}/><label style={{fontFamily:"'Nunito'",fontSize:13,fontWeight:600,color:T.textS,display:"block",marginBottom:8}}>Ville</label><div style={{display:"flex",gap:8}}><input value={city} onChange={e=>sC(e.target.value)} placeholder="Paris, Lyon..." style={{...inp,flex:1}}/><button onClick={detectCity} style={{padding:"12px 16px",borderRadius:14,background:T.accentSoft,border:`1px solid ${T.accent}33`,color:T.accent,fontSize:13,fontFamily:"'Nunito'",fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",height:"fit-content"}}>📍</button></div></>}
...
{st===2&&<><label style={{fontFamily:"'Nunito'",fontSize:13,fontWeight:600,color:T.textS,display:"block",marginBottom:8}}>Bio</label><textarea value={bio} onChange={e=>sB(e.target.value.slice(0,150))} placeholder="Décris-toi..." rows={4} style={{...inp,resize:"none",lineHeight:1.6}}/></>}
{st===3&&<><label style={{fontFamily:"'Nunito'",fontSize:13,fontWeight:600,color:T.textS,display:"block",marginBottom:14}}>Photo</label><div onClick={()=>fr.current?.click()} style={{border:`2px dashed ${T.borderL}`,borderRadius:20,padding:pp?0:40,textAlign:"center",cursor:"pointer",overflow:"hidden"}}>{pp?<img src={pp} alt="" style={{width:"100%",maxHeight:260,objectFit:"cover",display:"block",borderRadius:18}}/>:<div><div style={{fontSize:48,marginBottom:8}}>📸</div><p style={{fontFamily:"'Nunito'",fontSize:14,color:T.textS}}>Clique pour choisir</p></div>}</div><input ref={fr} type="file" accept="image/*" onChange={e=>{const f=e.target.files?.[0];if(!f)return;sPf(f);sErr("");const r=new FileReader();r.onload=ev=>sPp(ev.target.result);r.readAsDataURL(f)}} style={{display:"none"}}/></>}
</Card>
...
// ═══ HOME ═══
function HomeTab({profile,onStart,onSpeed,bonuses}){const T=useT();return<div style={{padding:20,maxWidth:440,margin:"0 auto",paddingBottom:90,animation:"fadeIn .5s"}}>
<div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>{profile.photos?.[0]&&<img src={profile.photos[0]} alt="" style={{width:52,height:52,borderRadius:"50%",objectFit:"cover",border:`2.5px solid ${T.accent}`}}/>}<div style={{flex:1}}><div style={{fontFamily:"'Nunito'",fontSize:17,fontWeight:800,color:T.text}}>Salut {profile.name} 👋</div><div style={{fontFamily:"'Nunito'",fontSize:12,color:T.textD}}>{profile.city} · {profile.age} ans</div></div></div>
<Card style={{padding:"14px 18px",marginBottom:16}}><XPBar xp={profile.xp||0}/></Card>
<Btn full onClick={onStart} style={{padding:22,fontSize:18,marginBottom:12,animation:"glow 2s ease-in-out infinite"}}>⚡ Conversation (60s)</Btn>
<Btn variant="sec" full onClick={onSpeed} style={{padding:18,fontSize:16,marginBottom:24}}>🚀 Speed Dating (3×30s)</Btn>
<Card style={{padding:20,marginBottom:20}} hover><h3 style={{fontFamily:"'Nunito'",fontSize:11,fontWeight:800,color:T.textD,margin:"0 0 14px",textTransform:"uppercase",letterSpacing:2}}>🎁 Bonus</h3><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{BONUS_TYPES.map(b=><div key={b.id} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:14,background:T.surfAlt,border:`1px solid ${T.border}`}}><span style={{fontSize:16}}>{b.icon}</span><span style={{fontFamily:"'Nunito'",fontSize:12,fontWeight:700,color:T.text}}>{b.name}</span><span style={{fontFamily:"'Nunito'",fontSize:14,fontWeight:800,color:(bonuses?.[b.id]||0)>0?T.gold:T.textD,marginLeft:4}}>{bonuses?.[b.id]||0}</span></div>)}</div></Card></div>}

// ═══ MATCH CHAT — COMPLETELY STANDALONE FULLSCREEN ═══
function MatchChatView({T,myUid,match,onBack,onBlock,onReport}){
...
useEffect(()=>{
const q=query(collection(db,"matches",match.matchId,"messages"),orderBy("createdAt","asc"));
const unsub=onSnapshot(q,snap=>{sM(snap.docs.map(d=>({id:d.id,...d.data()})))});
return()=>unsub();
},[match.matchId]);
...
await addDoc(collection(db,"matches",match.matchId,"messages"),{senderId:myUid,text:t,createdAt:serverTimestamp()});
await updateDoc(doc(db,"matches",match.matchId),{lastMessage:t});
}

return<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:100,background:T.bg,display:"flex",flexDirection:"column"}}>
{showReport&&<ReportModal T={T} onClose={()=>setShowReport(false)} onReport={async(r)=>{await addDoc(collection(db,"reports"),{reporter:myUid,reported:match.otherId,reason:r,createdAt:serverTimestamp()});setShowReport(false);alert("Signalement envoyé")}}/>}
...
<button onClick={onBack} style={{background:"transparent",border:"none",color:T.textS,fontSize:22,cursor:"pointer",padding:0}}>←</button>
{match.photos?.[0]&&<img src={match.photos[0]} alt="" style={{width:40,height:40,borderRadius:"50%",objectFit:"cover",border:`2px solid ${T.accent}`}}/>}
<div style={{flex:1}}><div style={{fontFamily:"'Nunito'",fontSize:15,fontWeight:700,color:T.text}}>{match.name}</div></div>
<button onClick={()=>setShowReport(true)} style={{background:"transparent",border:"none",fontSize:18,cursor:"pointer",padding:4}}>🚩</button>
<button onClick={onBlock} style={{background:"transparent",border:"none",fontSize:18,cursor:"pointer",padding:4}}>🚫</button>
</div>

{/* Messages */}
<div ref={scrollRef} style={{flex:1,overflowY:"auto",padding:16}}>
{msgs.length===0&&<div style={{textAlign:"center",padding:40,fontFamily:"'Nunito'",color:T.textD,fontSize:14}}>
<div style={{fontSize:40,marginBottom:12}}>💬</div>
Envoie le premier message !
</div>}
{msgs.map(m=><Msg key={m.id} m={m} myUid={myUid} T={T}/>)}
</div>

{/* Input — always visible at bottom */}
<div style={{padding:"10px 14px",borderTop:`1px solid ${T.border}`,background:T.surface,display:"flex",gap:8,flexShrink:0}}>
<input ref={inputRef} value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")sendMsg()}} placeholder="Message..." autoFocus
...
<button onClick={sendMsg} style={{width:46,height:46,borderRadius:14,border:"none",background:text.trim()?T.accentGrad:T.input,color:"#fff",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:text.trim()?`0 4px 16px ${T.accentGlow}`:"none"}}>↑</button>
</div>
</div>
}

// ═══ MATCHES TAB ═══
function MatchesTab({myUid,matches,onBlock}){const T=useT();
const[selectedMatch,setSelectedMatch]=useState(null);

if(selectedMatch)return<MatchChatView T={T} myUid={myUid} match={selectedMatch}
  onBack={()=>setSelectedMatch(null)}
  onBlock={async()=>{if(confirm(`Bloquer ${selectedMatch.name} ?`)){await onBlock(selectedMatch.otherId);setSelectedMatch(null)}}}
  onReport={()=>{}}/>;
...
{matches.map(m=><Card key={m.matchId} hover style={{padding:"16px 20px",marginBottom:12,cursor:"pointer"}}><div onClick={()=>setSelectedMatch(m)} style={{display:"flex",alignItems:"center",gap:14}}>
...
async function send(){if(!text.trim())return;const t=text.trim();setText("");ir.current?.focus();await addDoc(collection(db,"blindChats",chatId,"messages"),{senderId:myUid,text:t,createdAt:serverTimestamp()})}

return<div style={{display:"flex",flexDirection:"column",height:"100vh",maxWidth:440,margin:"0 auto"}}>
{showReport&&<ReportModal T={T} onClose={()=>setShowReport(false)} onReport={async(r)=>{await onReport(r);setShowReport(false)}}/>}
...
<div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:40,height:40,borderRadius:"50%",background:T.accentGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:800,color:"#fff",fontFamily:"'Nunito'"}}>?</div><div><div style={{fontFamily:"'Nunito'",fontSize:14,fontWeight:700,color:T.text}}>Inconnu·e</div><div style={{fontFamily:"'Nunito'",fontSize:10,color:T.success}}>● En ligne</div></div></div>
<div style={{display:"flex",alignItems:"center",gap:8}}>
<button onClick={()=>setShowReport(true)} style={{background:"transparent",border:"none",fontSize:16,cursor:"pointer",padding:2}} title="Signaler">🚩</button>
...
{BONUS_TYPES.map(b=>{const c=bonuses[b.id]||0;return<button key={b.id} disabled={c<=0} onClick={()=>useBonus(b.id)} style={{display:"flex",alignItems:"center",gap:4,padding:"5px 10px",borderRadius:12,border:`1px solid ${c>0?T.gold+"44":T.border}`,background:c>0?T.goldSoft:"transparent",cursor:c>0?"pointer":"not-allowed",opacity:c>0?1:.4,whiteSpace:"nowrap",flexShrink:0,fontSize:13}}><span>{b.icon}</span><span style={{fontFamily:"'Nunito'",fontSize:11,fontWeight:700,color:c>0?T.gold:T.textD}}>{c}</span></button>})}</div>
...
{msgs.map(m=><Msg key={m.id} m={m} myUid={myUid} T={T}/>)}</div>
<div style={{padding:"10px 14px",borderTop:`1px solid ${T.border}`,background:T.surface,display:"flex",gap:8,flexShrink:0}}>
<input ref={ir} value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Écris quelque chose..." autoFocus style={{flex:1,padding:"12px 18px",borderRadius:16,background:T.input,border:`1.5px solid ${T.border}`,color:T.text,fontSize:14,fontFamily:"'Nunito'",outline:"none"}}/>
<button onClick={send} style={{width:46,height:46,borderRadius:14,border:"none",background:text.trim()?T.accentGrad:T.input,color:"#fff",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>↑</button></div></div>}
...
if(myUid<p.uid){const et=new Date(Date.now()+CHAT_DUR*1000).toISOString();const ref=await addDoc(collection(db,"blindChats"),{users:[myUid,p.uid],status:"active",user1Decision:null,user2Decision:null,endTime:et,createdAt:serverTimestamp()});
await deleteDoc(doc(db,"waitingRoom",myUid)).catch(()=>{});await deleteDoc(doc(db,"waitingRoom",p.uid)).catch(()=>{});
await addXP(XP_CHAT);sCid(ref.id);sOuid(p.uid);sScr("chat")}else{
const cq=query(collection(db,"blindChats"),where("users","array-contains",myUid),where("status","==","active"));
const cu=onSnapshot(cq,cs=>{const f=cs.docs.find(d=>d.data().users.includes(p.uid));if(f){cu();addXP(XP_CHAT);sCid(f.id);sOuid(p.uid);sScr("chat")}});setTimeout(()=>cu(),20000)}})}
...
async function reportInBlind(reason){await addDoc(collection(db,"reports"),{reporter:user.uid,reported:ouid,reason,chatId:cid,createdAt:serverTimestamp()});alert("Signalement envoyé")}
...
{scr==="main"&&prof&&<>{tab==="home"&&<HomeTab profile={prof} onStart={startChat} onSpeed={()=>sScr("speed")} bonuses={bonuses}/>}{tab==="matches"&&<MatchesTab myUid={user.uid} matches={matches} onBlock={blockUser}/>}{tab==="profile"&&<ProfileTab user={user} profile={prof} setProfile={sProf} onLogout={()=>signOut(auth)}/>}<NavBar tab={tab} setTab={sTab} n={matches.length}/></>}
{scr==="waiting"&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",padding:40,textAlign:"center"}}><div style={{width:64,height:64,border:`3px solid ${T2.border}`,borderTopColor:T2.accent,borderRadius:"50%",animation:"spin 1s linear infinite",marginBottom:28}}/><h2 style={{fontFamily:"'Playfair Display',serif",fontSize:24,color:T2.text,margin:"0 0 10px"}}>Recherche...</h2><p style={{fontFamily:"'Nunito'",fontSize:14,color:T2.textS,marginBottom:36}}>On cherche quelqu'un !</p><Btn variant="ghost" onClick={cancelW}>Annuler</Btn></div>}
{scr==="chat"&&cid&&<BlindChat chatId={cid} myUid={user.uid} otherUid={ouid} partner={opartner} bonuses={bonuses} onUseBonus={consumeBonus} onTimeUp={onTU} onReport={reportInBlind}/>}
...
{scr==="speed"&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",padding:40,textAlign:"center"}}><div><div style={{fontSize:56,marginBottom:20}}>🚀</div><h2 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:T2.text,marginBottom:12}}>Speed Dating</h2><p style={{fontFamily:"'Nunito'",fontSize:14,color:T2.textS,marginBottom:32}}>Mode en construction !<br/>Bientôt disponible.</p><Btn onClick={()=>sScr("main")}>Retour</Btn></div></div>}
</div></TC.Provider>}